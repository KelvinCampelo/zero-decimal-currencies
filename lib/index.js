...
exports.default = function (amount, currency, display, noRound) {
  try {
    if (!amount) {
      throw new _errorFactory2.default('The amount value is required');
    }
    if (!currency) {
      throw new _errorFactory2.default('The currency value is required');
    }
    //test if number can be converted to float
    if (!isNaN(amount) && amount) {
      amount = parseFloat(amount.toString());
    } else {
      throw new _errorFactory2.default('The amount cannot be parsed to Float');
    }
    ...
    console.log(error);
    throw new _errorFactory2.default();
  }
};
